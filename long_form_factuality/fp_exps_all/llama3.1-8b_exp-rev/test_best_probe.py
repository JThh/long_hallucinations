...
def initialize_model():
    """Initialize the tokenizer and model for Llama3.1-8B."""
...
def test_on_new_data(model_name, tokenizer, llm, probe_summaries, test_data_dir=DEFAULT_TEST_DATA_DIR, results_dir=RESULTS_DIR):
    ...
    for test_file in test_files:
        ...
        for probe_info in probe_summaries:
            classifier_name = probe_info.get('classifier_name', 'default')  # e.g., 'logistic_regression_C0.5_group5'
            C_value = probe_info.get('C', 'default')
            group_size = probe_info.get('group_size', 'default')
            identifier = probe_info.get('training_file', 'default')
            logger.info(f'\n--- Testing Classifier: {classifier_name}, Identifier={identifier}, C={C_value}, group_size={group_size} on Dataset: {test_file} ---')
    ...
    if test_results:
        try:
            ...
            test_results_filepath = os.path.join(results_dir, f'{model_name}_{identifier}_test_aurocs.csv')
    ...
    if time_benchmarks_test:
        try:
            ...
            time_benchmarks_filepath = os.path.join(results_dir, f'{model_name}_{identifier}_test_time_benchmarks.csv')
...
def main():
    parser = argparse.ArgumentParser(description='Probing Experiments with Logistic Regression and XGBoost for Llama3.1-8B')
    ...
    for train_file in training_files:
        ...
        for settings in logistic_settings:
            ...
            for group_size in group_sizes:
                for classifier_name in ['logistic_regression', 'xgboost']:
                    files = os.listdir(RESULTS_DIR)
                    # import pdb; pdb.set_trace()
                    for file in files:
                        if file.startswith(f'{MODEL_NAME}_best_probe_layers_') and file.endswith(f'_{classifier_name}_C{C_value}_group{group_size}_{identifier}.pkl'):
                            with open(os.path.join(RESULTS_DIR, file), 'rb') as f:
                                summary = pickle.load(f)
                            summary['training_file'] = identifier
