...
def analyze_run(wandb_runid, assign_new_wandb_id=False, answer_fractions_mode='default'):
    ...
    for measure_name, measure_values in results_old['uncertainty_measures'].items():
        ...
        for validation_is_false, logging_name in zip(validation_is_falses, logging_names):
            ...
            # Can we use certainty (measure values) to predict if the model is false (validation_is_false)?
            # (Higher measure_values should be predictive of with validation_is_false.)
            fpr, tpr, _ = roc_curve(validation_is_false, measure_values)
            auroc = auc(fpr, tpr)

            result_dict[wandb_runid][name]['AUROC'] = auroc

            logging.info("AUROC for measure name `%s`: %f", name, auroc)

            # Compute 95% quantile of predictive entropy.
            for answer_fraction in answer_fractions:

                # Get mask of indices where predictive entropy is below 95% quantile.
                mask = np.array(measure_values) < np.quantile(
                    measure_values, answer_fraction)

                # Compute the mean of the values of validation_is_false where mask is True.
                accuracy = 1 - np.mean(np.array(validation_is_false)[mask])

                result_dict[wandb_runid][name][
                    f'accuracy_at_{answer_fraction:.2f}_answer_fraction'] = accuracy
